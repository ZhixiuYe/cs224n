...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        #self.input_placeholder = tf.placeholder([self.config.batch_size, self.config.n_features], tf.float32)
        #self.labels_placeholder = tf.placeholder([self.config.batch_size, self.config.n_classes], tf.float32)
        self.input_placeholder = tf.placeholder(dtype=tf.float32, shape=[self.config.batch_size, self.config.n_features], name="input")
        self.labels_placeholder = tf.placeholder(dtype=tf.float32, shape=[self.config.batch_size, self.config.n_classes], name="labels")
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        b = tf.expand_dims(tf.Variable(tf.zeros((self.config.n_classes)), name="b"), 0)
...
def test_softmax_model():
    ...
    with tf.Graph().as_default():
        ...
        # 这里传入softmaxmodel得config传入了__init__函数中，
