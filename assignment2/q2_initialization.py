...
def xavier_weight_init():
    ...
    def _xavier_initializer(shape, **kwargs):
        ...
        """
        ### YOUR CODE HERE
        epsilon = tf.sqrt(6.0 / tf.reduce_sum([float(x) for x in shape]))
        out = tf.Variable(tf.random_uniform(shape=shape,minval=-epsilon,maxval=epsilon, **kwargs), dtype=tf.float32)
        ### END YOUR CODE
        return out
