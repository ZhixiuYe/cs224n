...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ### YOUR CODE HERE
        self.stack = ["ROOT"]
        self.buffer = copy.copy(sentence)
        self.dependencies = []
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while 1:
        if unfinished_parses == []:
            break
        if len(unfinished_parses) <= batch_size:
            batch_parses = unfinished_parses[:]
        else:
        ...
        unfinished_parses = filter(lambda parse:len(parse.stack) != 1 or len(parse.buffer) != 0, unfinished_parses)
