...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x = x / (np.sqrt(np.sum(x**2, axis=1,keepdims=True)) + 1e-10)
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    probabilies = softmax(predicted.dot(outputVectors.T))
    cost = -1 * np.log(probabilies[target])
    delta = probabilies
    delta[target] -= 1
    N = delta.shape[0]
    D = predicted.shape[0]
