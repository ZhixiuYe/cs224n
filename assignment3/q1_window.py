...
class Config:
    ...
    window_size = 5 # The size of the window to use.
    ### YOUR CODE HERE
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        full_sentence = [start] * window_size + sentence + [end] * window_size
        for s in range(len(sentence)):
            xlist = full_sentence[s : s + 2 * window_size + 1]
            x = sum(xlist,[])
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(shape=(None, self.config.n_window_features), dtype=tf.int32, name="input")
        self.labels_placeholder = tf.placeholder(shape=[None,], dtype=tf.int32, name="labels")
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        embedding_dict = self.pretrained_embeddings
        embedding_tensor = tf.nn.embedding_lookup(embedding_dict, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name="W", initializer=tf.contrib.layers.xavier_initializer(), shape=(self.config.n_window_features * self.config.embed_size, self.config.hidden_size))
        b1 = tf.get_variable(name="b1", initializer=tf.contrib.layers.xavier_initializer(), shape=[self.config.hidden_size])
        U = tf.get_variable(name="U", initializer=tf.contrib.layers.xavier_initializer(), shape=[self.config.hidden_size, self.config.n_classes])
        b2 = tf.get_variable(name="b2", initializer=tf.contrib.layers.xavier_initializer(), shape=[self.config.n_classes])

        pred = tf.matmul(tf.nn.dropout(tf.nn.relu(tf.matmul(x, W) + b1), self.dropout_placeholder), U) + b2
