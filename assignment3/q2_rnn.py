...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) < max_length:
            sentencel = sentence + [zero_vector] * (max_length - len(sentence))
            labelsl = labels + [zero_label] * (max_length - len(sentence))
            mask = [True] * len(sentence) + [False] * (max_length - len(sentence))
            ret.append((sentencel, labelsl, mask))

        else:
            sentencel = sentence[:max_length]
            labelsl = labels[:max_length]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embed = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable("b2", initializer=[0.] * self.config.n_classes)
        old_state = tf.zeros((tf.shape(x)[0],self.config.hidden_size))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                old_state = new_state
                out_dropout = tf.nn.dropout(out, dropout_rate)
                yhat = tf.matmul(out_dropout, U) + tf.expand_dims(b2, 0)
                #preds = tf.concat(1, [preds, tf.expand_dims(yhat, 1)])
                preds.append(yhat)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        #slice_preds = tf.boolean_mask(preds, self.mask_placeholder)
        all_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds, labels=self.labels_placeholder)
        loss = tf.reduce_mean(tf.boolean_mask(all_loss, self.mask_placeholder))
        ### END YOUR CODE
        return loss
